...
def _make_inputs(B, T, Hk, Hv, Dk, Dv, dtype):
    ...
    mx.random.seed(42)
    scale = 0.1
    q = (mx.random.normal((B, T, Hk, Dk)) * scale).astype(dtype)
    k = (mx.random.normal((B, T, Hk, Dk)) * scale).astype(dtype)
    v = (mx.random.normal((B, T, Hv, Dv)) * scale).astype(dtype)
    a = (mx.random.normal((B, T, Hv)) * scale).astype(dtype)
    b = (mx.random.normal((B, T, Hv)) * scale).astype(dtype)
    A_log = (mx.random.normal((Hv,)) * scale).astype(dtype)
    dt_bias = (mx.random.normal((Hv,)) * scale).astype(dtype)
    state = mx.zeros((B, Hv, Dv, Dk), dtype=dtype)
    mx.eval(q, k, v, a, b, A_log, dt_bias, state)
    return q, k, v, a, b, A_log, dt_bias, state


def bench_one(*, backend, B, T, Hk, Hv, Dk, Dv, warmup, iters, dtype) -> float:
    ...
    q, k, v, a, b, A_log, dt_bias, state = _make_inputs(B, T, Hk, Hv, Dk, Dv, dtype)

    if backend == "fused":
        from vllm_metal.metal.linear_attention import fused_gdn_decode

        fn = lambda: fused_gdn_decode(q, k, v, a, b, A_log, dt_bias, state)
    elif backend == "metal":
        ...
        beta = mx.sigmoid(b)
        mx.eval(g, beta)
        fn = lambda: gated_delta_kernel(q, k, v, g, beta, state)
    elif backend == "ops":
        def _ops_fn():
            g = compute_g(A_log, a, dt_bias)
            beta = mx.sigmoid(b)
...
def check_correctness(Hv=32, dtype=mx.float16):
    ...
    print(f"Correctness check (Hv={Hv}, dtype={dtype})...")

    for B, T in [(1, 1), (1, 16), (4, 1), (2, 8)]:
        q, k, v, a, b, A_log, dt_bias, state = _make_inputs(
            B, T, HK, Hv, DK, DV, dtype
        )
        ...
        print(f"  B={B} T={T}: y_maxabs={y_diff:.6f} s_maxabs={s_diff:.6f} [{status}]")
...
def main():
    ...
    ok = check_correctness(Hv=args.hv[0], dtype=dtype)
    if not ok:
    ...
    for Hv in args.hv:
        for B in args.batch:
            for T in args.seq_lens:
                common = dict(
                    B=B, T=T, Hk=HK, Hv=Hv, Dk=DK, Dv=DV,
